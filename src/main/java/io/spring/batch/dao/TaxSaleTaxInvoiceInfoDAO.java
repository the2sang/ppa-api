...
import io.spring.batch.common.CommProperties;
...
@Configuration
public class TaxSaleTaxInvoiceInfoDAO {

    String getSaveSaleTaxInvoiceInfoTBSQL() {

        String DB_ENC = CommProperties.getString("DB_ENC"); // 암호화 처리 여부 정보 가져오기.
        ...
        if (DB_ENC.equals("1")) {
            sb.append("\n MERGE INTO TB_TAX_BILL_INFO_ENC DAT");
        } else {
            sb.append("\n MERGE INTO TB_TAX_BILL_INFO DAT");
        }
        //수정 유종일
    ...
    @Bean
    ItemWriter<TaxSaleTaxInvoiceMgrVO> TaxSaleTaxInvoiceInfoVOWriter(DataSource dataSource,
                                                                      NamedParameterJdbcTemplate jdbcTemplate) {
        JdbcBatchItemWriter<TaxSaleTaxInvoiceMgrVO> databaseItemWriter = new JdbcBatchItemWriter<>();
        databaseItemWriter.setDataSource(dataSource);
        databaseItemWriter.setJdbcTemplate(jdbcTemplate);

        databaseItemWriter.setSql(getSaveSaleTaxInvoiceInfoTBSQL());

        ItemPreparedStatementSetter<TaxSaleTaxInvoiceMgrVO> valueSetter =
                new TaxSaleTaxInvoiceInfoDAOSetter();
    ...
    @Bean
    String getSelectSQLSyntaxFromTaxEmailBillInfoList() {
        ...
        sb.append("\n      , INVOICER_PARTY_ID");
        sb.append("\n      , GRAND_TOTAL_AMOUNT");
        sb.append("\n      , GET_CODE_NAME('BILL_TYPE_CODE',BILL_TYPE_CODE) BILL_TYPE_CODE_NM"); //테이블 및 function 생성 필요
        sb.append("\n	   , MAIL_STATUS_CODE");
        sb.append("\n	   , MAIL_GUB_CODE");
    ...
    @Bean
    String getFromSQLSyntaxFromTaxEmailBillInfo() {
        return "\n   FROM TAX_EMAIL_BILL_INFO";
    }

    String getWhereClauseFromTaxEmailBillInfoLIst() {
        StringBuffer sb = new StringBuffer();

        sb.append("\n    WHERE (MAIL_STATUS_CODE IS NULL OR MAIL_STATUS_CODE = '98')");
